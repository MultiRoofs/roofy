...
- `LEVEL_SWAP_TIMEOUT_MS = 1500` (`navara-flatcitybuf/src/constants.ts`) is tuned for real hardware. On a GPU-less SwiftShader host (~3 fps) the first post-fit commit blows the deadline and recovers on the next settle; re-measure before changing it.
