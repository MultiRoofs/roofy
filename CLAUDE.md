...
- One lighting calibration: the engine's own forward-lit default (`view.lit = true`, the aerial-perspective pass at its `irradiance: false` default) at exposure 10, lit by the photoreal scene's sun (cascaded shadow maps, `SUN_SHADOW_TUNING`) and sky light probe, with no app-added scene lights. City meshes are ONE lit material (`createCityMaterial` in `navara-cityjson`: Lambert, vertex colours, double-sided) drawn in the engine's MRT scene and registered for shadows through the `ShadowMaterialHooks` seam; both mesh classes and the textured-group builder must agree. Themes darken with sun intensity, probe intensity and exposure, never with a second lighting pass.
