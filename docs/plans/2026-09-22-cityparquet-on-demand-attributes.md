...
| `src/features/streaming/useObjectAttributes.ts` (new)        | Hook: `{status:"complete"                                                                                                                                                                                | "loading" | "error", attributes}` for a selected object of a streaming layer; cancels on change; never blocks the render of what is already known. |
