...
> **STATUS (2026-09-22): deferred behind task 5, and to be revised before execution.**
> The Codex review of this plan (kept at `.superpowers/sdd/notes/codex-plan4-review.txt`) found one Critical and seven Important problems, most of which share a root cause: today every consumer of a streamed layer's attributes — the DuckDB table, attribute filters, the legend's rule counts, export, the processing toolbox, and the worker's own rule colouring and derived `volumeCuM` — reads them from the RESIDENT records. Removing columns from those records therefore degrades all of them at once, and a rule added after load only triggers a recolour from the cached model (`streamLayer.ts` `setRules` → `recolorCells`), not a refetch, so it would style objects from attributes that were never read.
> Task 5 (object families and per-family tables) removes that root cause: each family's table is read from the FILE — every row, every attribute column — independent of what is resident. With the table as the complete source, deferring columns in the render path only affects the worker's own needs (rule dependencies and derived values), and the inspector can read an object's attributes from its family table.
> **Ruling: implement task 5 first, then revise this plan** against the shipped task 5 and the review's findings: dependency-driven projection (rules use `conditions[].field`, physical names differ — `+name` extension columns and keys inside `other_attributes`), invalidate-and-refetch when the dependency set grows, derived-value dependencies (`measuredHeight` → `volumeCuM`), family/ancestor hydration for inherited attributes, per-request buffers or a queue (the reader requires serialised callers), the row map's eviction and overflow behaviour, source-version validation, and byte budgets for attribute reads and the cache.
