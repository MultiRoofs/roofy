...
**Task 4 — attributes on demand.** Gate: two separate benchmarks on the real Yokohama file, because they are different workloads — (a) ONE object's attributes: worker row-location read (`rowStart = row, rowEnd = row+1, useOffsetIndex`, attribute columns only; task 3's `StreamRow` gives `{table,row}`) vs DuckDB-wasm `read_parquet` over an HTTP-registered file with explicit columns and `WHERE id = ?`; (b) a WHOLE family's attribute columns (for task 5's tables): DuckDB `read_parquet` projected scan vs worker reads inserted in chunks. Record bytes and latency; repeated single-row reads decode whole pages, so a small per-page cache may be needed. Then: streamed CityParquet reads identity + geometry + rule-referenced attribute columns only; a new `attributes` worker message returns one object's attribute columns; the inspector shows the object at once (selection and highlight are id-only already) and fills attributes on arrival. `useResolvedBuilding.loading` today means "family members not resident" — attribute readiness gets its OWN state. Bounded LRU (512 objects) keyed by object id + source identity (URL + size/`ETag`).
