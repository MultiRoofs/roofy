/**
 * Measure solids driven through a REAL run: the real executor, the real queue,
 ...
 *
 * `measureSolids.test.ts` covers the tool's own behaviour over a stubbed
 ...
 *
 * The scaffolding is `roofMetricsRun.test.ts`'s — the same duckdb and
 * `layerTables` mocks, the same gate, the same FIFO chain — because the queue
 * this runs on is the same queue. What differs is the TABLE: this one is
 * READER-BACKED (a source provider, a reader and an LoD column), which is what
 * makes §6.1's "Reading source" phase real here.
 */
...
/** Rows the guarded measure statement answers with. */
...
vi.mock("../../../../src/insights/duckdb", () => {
  const run = async (statement: string) => {
    ...
    // The GUARDED measure statement, told apart from the id join by its parse.
...
/** One Measure solids request, as `ToolView` freezes it. */
...
/** The measure statement's answer for one valid solid. */
...
beforeEach(() => {
  ...
  measureRows = [solidAnswer("B1"), solidAnswer("B2")];
  gate = null;
...
describe("a Measure solids run", () => {
  ...
  it("releases the source and publishes NOTHING when cancelled mid-read", async () => {
    ...
    expect(attributesOf("B1").solid_volume_m3).toBeUndefined();
